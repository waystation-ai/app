  ...
  return (
    <NavigationMenu>
      <NavigationMenuList>
        ...
        <SignedIn>
        <NavigationMenuItem>
          <NavigationMenuLink className={navigationMenuTriggerStyle()} href="/chat" target="_blank">
            Chat
          </NavigationMenuLink>
        </NavigationMenuItem>
        </SignedIn>
    ...
    <div className="flex flex-col md:flex-row md:items-center flex-grow">
      <div className="flex w-full md:w-auto items-center">
        <Link className="flex items-center gap-2" href="/">
          <Image src="/images/logo.png" width={32} height={32} alt="WayStation" className="h-8 w-8" />
          <p className="text-2xl font-bold aurora-text">Escalator</p>
        </Link>
      ...
      <nav className={`md:hidden absolute top-full left-0 right-0 bg-white/95 backdrop-blur-xl shadow-lg transition-all duration-300 ease-in-out overflow-hidden ${mobileMenuOpen ? 'max-h-96 py-4' : 'max-h-0'}`}>
        <div className="flex flex-col gap-4 px-6">
          <div className="border-t border-gray-200 mt-2 pt-4">
            <Link href="/use-cases" className="apppp-link py-2" onClick={() => setMobileMenuOpen(false)}>Marketplace</Link>
            <Link href="/chat" className="app-link py-2" onClick={() => setMobileMenuOpen(false)}>Playground</Link>
          </div>
