...
export async function generateMetadata ({ params }: { params: Promise<{ provider: string }> }) {
  ...
  return {
    title: `Integrate LLM with ${config.name}`,
    description: `With WayStation plugin for LLMs you can ${config.description}`,
    openGraph: {
      type: 'article',
      title: `Integrate LLM with ${config.name}`,
      description: `With WayStation plugin for LLMs you can ${config.description}`,
      ...
      <main className="flex-1 flex flex-col">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 px-4 sm:px-8 py-8 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col justify-center text-left">
            ...
            Integrate your LLM with <span className="bg-yellow-100">{config.name}</span>
            </h1>
